...
enum PreferencesDispatchAction {
  ...
  // POE Beta Features
  USE_POE_FEATURES = 'preferences/usePOEFeatures',
  POE_TIME_PROOF_ENABLED = 'preferences/poeTimeProofEnabled',
  POE_LOCATION_PROOF_ENABLED = 'preferences/poeLocationProofEnabled',
  POE_UPDATE_LAST_TIME_PROOF = 'preferences/poeUpdateLastTimeProof',
  POE_UPDATE_LAST_LOCATION_PROOF = 'preferences/poeUpdateLastLocationProof',
}
...
export const reducer = <S extends State>(state: S, action: ReducerAction<DispatchAction>): S => {
  switch (action.type) {
    ...
    // POE Beta Features
    case PreferencesDispatchAction.USE_POE_FEATURES: {
      const choice = (action?.payload ?? []).pop() ?? false
      const preferences: Preferences = {
        ...state.preferences,
        usePOEFeatures: choice,
      }
      PersistentStorage.storeValueForKey(LocalStorageKeys.Preferences, preferences)
      return {
        ...state,
    ...
    case PreferencesDispatchAction.POE_TIME_PROOF_ENABLED: {
      const choice = (action?.payload ?? []).pop() ?? false
      const preferences: Preferences = {
        ...state.preferences,
        poeTimeProofEnabled: choice,
      }
      PersistentStorage.storeValueForKey(LocalStorageKeys.Preferences, preferences)
      return {
        ...state,
    ...
    case PreferencesDispatchAction.POE_LOCATION_PROOF_ENABLED: {
      const choice = (action?.payload ?? []).pop() ?? false
      const preferences: Preferences = {
        ...state.preferences,
        poeLocationProofEnabled: choice,
      }
      PersistentStorage.storeValueForKey(LocalStorageKeys.Preferences, preferences)
      return {
        ...state,
    ...
    case PreferencesDispatchAction.POE_UPDATE_LAST_TIME_PROOF: {
      const proofData = (action?.payload ?? []).pop()
      const preferences: Preferences = {
        ...state.preferences,
        poeLastTimeProof: proofData,
      }
      PersistentStorage.storeValueForKey(LocalStorageKeys.Preferences, preferences)
      return {
        ...state,
    ...
    case PreferencesDispatchAction.POE_UPDATE_LAST_LOCATION_PROOF: {
      const proofData = (action?.payload ?? []).pop()
      const preferences: Preferences = {
        ...state.preferences,
        poeLastLocationProof: proofData,
      }
      PersistentStorage.storeValueForKey(LocalStorageKeys.Preferences, preferences)
      return {
        ...state,
