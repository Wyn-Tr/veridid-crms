...
// ESSI API URL for fetching IACA certificates (for mdoc trust)
const ESSI_API_URL = 'https://api.essi.studio'

/**
 * Fetch IACA certificate from ESSI for mdoc verification
 * Returns the PEM certificate to be added to trusted certificates
 */
async function fetchEssiIacaCertificate(): Promise<string | null> {
  try {
    const response = await fetch(`${ESSI_API_URL}/.well-known/iaca-certificate`)
    if (!response.ok) {
      console.warn('[X509] Failed to fetch ESSI IACA certificate:', response.status)
      return null
    }
    const data = await response.json()
    return data.pem || null
  } catch (error) {
    console.warn('[X509] Error fetching ESSI IACA certificate:', error)
    return null
  }
}
...
const useBifoldAgentSetup = (): AgentSetupReturnType => {
  ...
  const createNewAgent = useCallback(
    async (walletSecret: WalletSecret, mediatorUrl: string): Promise<Agent> => {
      // Fetch ESSI IACA certificate for mdoc trust
      const essiCert = await fetchEssiIacaCertificate()
      const trustedCerts = essiCert ? [essiCert] : undefined
      if (essiCert) {
        logger.info('[X509] Loaded ESSI IACA certificate for mdoc verification')
      }

      const newAgent = new Agent({
        config: {
          label: store.preferences.walletName || 'Essi Wallet',
          walletConfig: {
            id: walletSecret.id,
            key: walletSecret.key,
          },
        ...
        modules: getAgentModules({
          ...
          trustedX509Certificates: trustedCerts,
        }),
      })
  ...
  const initializeAgent = useCallback(
    async (walletSecret: WalletSecret): Promise<void> => {
      ...
      logger.info(`[Env] MEDIATOR_URL: ${Config.MEDIATOR_URL ?? ''}`)
      logger.info(`[AgentSetup] selectedMediator: ${store.preferences.selectedMediator ?? ''}`)
