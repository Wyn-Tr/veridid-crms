...
export const useChatMessagesByConnection = (connection: ConnectionRecord): ExtendedChatMessage[] => {
  ...
  const { instances: workflows } = useWorkflows(connection?.id)
  ...
  useEffect(() => {
    ...
    transformedMessages.push(
      ...workflows.map((record: WorkflowInstanceRecord) => {
        // Workflows from the peer appear on the left, ours on the right
        const role = Role.them
        const userLabel = theirLabel
        const templateName = ((record as any).templateId ?? record.id)
          .replace(/[-_]/g, ' ')
          .replace(/\b\w/g, (c: string) => c.toUpperCase())
        const status = (record as any).status ?? 'Active'
        const actionLabel = `Started workflow: ${templateName}`

        return {
          _id: `workflow-${record.id}`,
          text: actionLabel,
          renderEvent: () => <ChatEvent role={role} userLabel={userLabel} actionLabel={`${templateName} (${status})`} />,
          createdAt: record.createdAt,
          type: 'WorkflowInstanceRecord',
          user: { _id: role },
          messageOpensCallbackType: CallbackType.Workflow,
          onDetails: () => {
            navigation.navigate(Screens.WorkflowDetails as any, { instanceId: record.id })
          },
        }
      })
    )
  ...
  }, [ColorPalette, basicMessages, theme, credentials, t, navigation, proofs, workflows, theirLabel, connection])
