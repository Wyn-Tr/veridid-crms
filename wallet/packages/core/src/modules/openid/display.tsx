  ...
  if (openId4VcMetadata) {
    ...
    if (openidIssuerDisplay) {
      ...
      if (openidIssuerDisplay.logo) {
        issuerDisplay.logo = {
          url: openidIssuerDisplay.logo?.url ?? (openidIssuerDisplay.logo as { uri?: string }).uri,
    ...
    if (openidCredentialDisplay && !issuerDisplay.logo && openidCredentialDisplay.logo) {
      issuerDisplay.logo = {
        url: openidCredentialDisplay.logo?.url ?? (openidCredentialDisplay.logo as { uri?: string }).uri,
...
function getCredentialDisplay(
  ...
  if (openId4VcMetadata) {
    ...
    credentialDisplay.backgroundImage = openidCredentialDisplay?.background_image
      ? {
        url:
          openidCredentialDisplay.background_image.url ??
          (openidCredentialDisplay.background_image as { uri?: string }).uri,
...
// ============================================================
// OpenBadges Detection & Display Utilities
// ============================================================

/**
 * Check if credential is an OpenBadge (OBv3)
 */
export function isOpenBadgeCredential(credential: W3cCredentialJson): boolean {
  return (
    credential.type?.includes('OpenBadgeCredential') ||
    credential.type?.includes('AchievementCredential')
  )
}

/**
 * OpenBadge-specific display data
 */
...
/**
 * Extract OpenBadge-specific display data from credential
 */
export function getOpenBadgeDisplayData(credential: W3cCredentialJson): OpenBadgeDisplayData {
  ...
  const issuer =
    typeof credential.issuer === 'string' ? { id: credential.issuer } : credential.issuer
...
function getW3cCredentialDisplay(
  ...
  // OpenBadge-specific handling - extract achievement info
  if (isOpenBadgeCredential(credential)) {
    const badge = getOpenBadgeDisplayData(credential)
    if (!credentialDisplay.name) {
      credentialDisplay.name = badge.achievementName
    }
    if (!credentialDisplay.description) {
      credentialDisplay.description = badge.achievementDescription
    }
    if (!credentialDisplay.logo && badge.achievementImage) {
      credentialDisplay.logo = { url: badge.achievementImage }
    }
  }
...
export function filterAndMapSdJwtKeys(sdJwtVcPayload: Record<string, unknown>) {
  ...
  const holder =
    typeof cnf?.kid === 'string'
      ? cnf.kid
      : cnf?.jwk
        ? safeCalculateJwkThumbprint(cnf.jwk as JwkJson)
        : undefined
