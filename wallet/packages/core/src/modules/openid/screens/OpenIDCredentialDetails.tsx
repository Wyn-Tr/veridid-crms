...
const OpenIDCredentialDetails: React.FC<OpenIDCredentialDetailsProps> = ({ navigation, route }) => {
  ...
  const { removeCredential, getW3CCredentialById, getSdJwtCredentialById, getMdocCredentialById, resolveBundleForCredential } =
  ...
  useEffect(() => {
    ...
    fetchCredential()
  }, [credentialId, type, getSdJwtCredentialById, getW3CCredentialById, getMdocCredentialById, agent, t, credentialRemoved])
  ...
  useEffect(() => {
    if (!credential || !credentialDisplay || !bundleResolver || !i18n || !credentialDisplay.display) {
      return
    }

    const resolveOverlay = async () => {
      const resolvedOverlay = await resolveBundleForCredential(credential)
      setOverlay(resolvedOverlay)
    }

    resolveOverlay()
  }, [credential, credentialDisplay, bundleResolver, i18n, resolveBundleForCredential])
  ...
  const renderOpenIdCard = () => {
    ...
    return <CredentialCard credential={credential} brandingOverlay={overlay} />
  }
