...
const HistorySettings: React.FC<HistorySettingsProps> = () => {
  ...
  const handleSaveHistorySettings = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        t('Global.Success'),
        t('ActivityHistory.SettingsSaved') || 'Your history settings have been saved.',
        [{ text: t('Global.Okay') }]
      )
    } catch (e: unknown) {
      // Show error alert
      ...
      logger.error(`[${HistorySettings.name}]: Handle history save: ${e}`)
    }
  }, [historyManager, logger, historyOptionSelected, initialHistory, t])
      ...
      <View style={style.screenContainer}>
        ...
        <View style={style.controlsContainer}>
          ...
          <Button
            ...
            testID={testIdWithKey('StopKeepingHistory')}
            accessibilityLabel={t('ActivityHistory.StopKeepingHistory')}
            buttonType={ButtonType.Secondary}
            onPress={() => {
              Alert.alert(
                t('ActivityHistory.StopActivityHistoryScreen.Title') || 'Stop Activity History?',
                t('ActivityHistory.StopActivityHistoryScreen.Description') || 'Your wallet will no longer keep activity history. This action cannot be undone.',
                [
                  { text: t('Global.Cancel'), style: 'cancel' },
                  {
                    ...
                    onPress: async () => {
                      if (historyManager) {
                        try {
                          await historyManager.handleStoreHistorySettings({ id: 'Never', key: 0, value: '0' })
                          Alert.alert(
                          ...
                          Alert.alert(t('Global.Failure'), (e as Error)?.message || t('Global.SomethingWentWrong'))
                        }
                      }
                    },
                  },
