...
import ESSIProofRequest from './essi/ESSIProofRequest'
import ESSICredentialOffer from './essi/ESSICredentialOffer'
...
const Connection: React.FC<ConnectionProps> = ({ navigation, route }) => {
  ...
  const logHistoryRecord = useCallback(() => {
    try {
      if (!(agent && historyEnabled)) {
        logger.trace(
          `[${ESSICredentialOffer.name}]:[logHistoryRecord] Skipping history log, either history function disabled or agent undefined!`
        )
        return
      }
      ...
      if (!connection) {
        logger.error(`[${ESSICredentialOffer.name}]:[logHistoryRecord] Cannot save history, credential undefined!`)
        return
      }
      ...
      logger.error(`[${ESSICredentialOffer.name}]:[logHistoryRecord] Error saving history: ${err}`)
    }
  ...
  const handleNavigation = useCallback(
    (connectionId: string) => {
      ...
      if (enableChat) {
        // Navigate to Chat within TabStack -> ContactStack to keep bottom nav visible
                  ...
                  routes: [
                    {
                      ...
                      state: {
                        routes: [
                          ...
                          { name: Screens.Chat, params: { connectionId } },
                        ],
                        index: 1,
                      },
                    },
                  ],
                  index: 1, // ContactStack is at index 1 in the Tab Navigator
                },
              },
            ],
          })
  ...
  useEffect(() => {
    ...
    if (
      ...
      (state.notificationRecord as { type?: string }).type === 'OpenId4VciPendingCredentialOffer'
    ) {
  ...
  useEffect(() => {
    ...
    for (const notification of notifications) {
      ...
      if (
        ...
        (notification as { type?: string }).type === 'OpenId4VciPendingCredentialOffer'
      ) {
        dispatch({ notificationRecord: notification })
        break
      }
  ...
  const displayComponent = () => {
    ...
    if (state.shouldShowProofComponent) {
      return <ESSIProofRequest proofId={proofId ?? state.notificationRecord.id} navigation={navigation} />
    }

    if (state.shouldShowOfferComponent) {
      return <ESSICredentialOffer credentialId={credentialId ?? state.notificationRecord.id} navigation={navigation} />
    }
  }
