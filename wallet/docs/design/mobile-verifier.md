...
Also, by default, there are two predefined templates set in Essi Wallet (`Student full name` and `Student full name and expiration date`) for testing purpose.
...
Essi Wallet Scanner updated to handle raw json messages -> pass them to Credo message receiver.
