...
const CredentialCard11: React.FC<CredentialCard11Props> = ({
  ...
  const gradientId = useMemo(() => `credential-gradient-${Math.random().toString(36).slice(2)}`, [])
  ...
  const cardData: Field[] = useMemo(() => {
    ...
    const primary = overlay.presentationFields?.find(
      f => f.name === overlay.brandingOverlay?.primaryAttribute
    )

    const secondary = overlay.presentationFields?.find(
      f => f.name === overlay.brandingOverlay?.secondaryAttribute
    )

    primary && fields.push(primary)
    secondary && fields.push(secondary)

    return displayItems ?? fields
  ...
  useEffect(() => {
    if (brandingOverlay) {
      setOverlay({
        ...(brandingOverlay as CredentialOverlay<BrandingOverlay>),
        brandingOverlay: normalizeBrandingOverlay(brandingOverlay?.brandingOverlay),
      })
      return
    }
  ...
  function normalizeBrandingOverlay(
    overlay?: BrandingOverlay | any
  ): BrandingOverlay | undefined {
    if (!overlay) return overlay

    // Mutate in place to preserve class + private fields
    if (!overlay.primaryBackgroundColor && overlay.primary_background_color) {
      overlay.primaryBackgroundColor = overlay.primary_background_color
    }

    if (!overlay.secondaryBackgroundColor && overlay.secondary_background_color) {
      overlay.secondaryBackgroundColor = overlay.secondary_background_color
    }

    if (!overlay.backgroundImage && overlay.background_image) {
      overlay.backgroundImage = overlay.background_image
    }

    if (!overlay.primaryAttribute && overlay.primary_attribute) {
      overlay.primaryAttribute = overlay.primary_attribute
    }

    if (!overlay.secondaryAttribute && overlay.secondary_attribute) {
      overlay.secondaryAttribute = overlay.secondary_attribute
    }

    if (!overlay.svgTemplateUrl && overlay.svg_template_url) {
      overlay.svgTemplateUrl = overlay.svg_template_url
    }

    if (!overlay.svgBindings && overlay.svg_bindings) {
      overlay.svgBindings = overlay.svg_bindings
    }

    return overlay as BrandingOverlay
  }
  ...
  const CredentialCard: React.FC<{ status?: 'error' | 'warning' }> = ({ status }) => {
    ...
    const baseBackgroundColor =
      overlay.brandingOverlay?.primaryBackgroundColor ?? '#1e3a5f'

    const secondaryBackgroundColor =
      overlay.brandingOverlay?.secondaryBackgroundColor

    const hasGradient = Boolean(secondaryBackgroundColor) 

    // const hasGradientBackground = Boolean(overlay.brandingOverlay?.secondaryBackgroundColor)
      ...
      <View style={styles.cardContainer} accessible={true} accessibilityLabel={accessibilityLabel}>
        ...
        <View
          style={[
            ...
            { borderRadius, backgroundColor: baseBackgroundColor },
          ]}
        />
          ...
          <Svg style={StyleSheet.absoluteFillObject}>
            <Defs>
              <LinearGradient id={gradientId} x1="0%" y1="0%" x2="100%" y2="100%">
                <Stop offset="0%" stopColor={baseBackgroundColor} />
                <Stop offset="100%" stopColor={secondaryBackgroundColor!} />
              </LinearGradient>
            </Defs>
            <Rect
              width="100%"
        ...
        {/* Background image (ESSI puts it ON TOP of gradient) */}
