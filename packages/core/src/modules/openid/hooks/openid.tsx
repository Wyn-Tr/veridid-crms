...
import { OpenId4VciPendingCredentialOffer, OpenId4VPRequestRecord } from '../types'
...
export const useOpenID = ({
  ...
  const [openIdRecord, setOpenIdRecord] = useState<
    SdJwtVcRecord | W3cCredentialRecord | MdocRecord | OpenId4VPRequestRecord | OpenId4VciPendingCredentialOffer
  >()
  ...
  const resolveOpenIDCredential = useCallback(
    async (uri: string) => {
      ...
      try {
        ...
        const preAuthGrant = offerPayload?.grants?.[
